...
export async function catalogNew(options: NewOptions = {}) {
  ...
  const gitUrl = options.gitUrl || await input({ 
...
`;
  ...
  logger.info(`  1. Add skills: npx @bre/ai-setup catalog skill add <name>`);
  logger.info(`  2. Validate:   npx @bre/ai-setup catalog validate`);
  logger.info(`  3. Build:      npx @bre/ai-setup catalog build`);
}
