...
export async function sync(options: SyncOptions = {}) {
  ...
  let gitCatalogs = Object.entries(config.catalogs).filter(
    ([_, entry]) => entry.type === "git",
  );
  ...
  if (options.catalog) {
    ...
    if (gitCatalogs.length === 0) {
      logger.error(
        `Catalog "${options.catalog}" not found or is not a Git catalog`,
      );
      process.exit(1);
    }
  }
  ...
  for (const [catalogId, entry] of gitCatalogs) {
    try {
      ...
      if (result.success) {
        console.log(
          chalk.green(`✓ ${catalogId}`) + chalk.dim(` (${entry.url})`),
        );
        ...
        console.log(
          chalk.yellow(`⚠ ${catalogId}`) + chalk.dim(` - ${result.error}`),
        );
        failCount++;
      }
    } catch (error) {
      console.log(
        chalk.red(`✖ ${catalogId}`) +
          chalk.dim(` - ${(error as Error).message}`),
      );
      failCount++;
    }
  }
