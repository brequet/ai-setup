...
export async function list() {
  ...
  if (catalogEntries.length === 0) {
    ...
    console.log('');
    logger.info('Add a catalog: npx @bre/ai-setup add <catalog-path>');
    return;
  }
  ...
  for (const [id, entry] of catalogEntries) {
    try {
      ...
      if (catalogUpdates > 0 || catalogNew > 0) {
        ...
        if (catalogUpdates > 0) parts.push(`${catalogUpdates} update${catalogUpdates === 1 ? '' : 's'}`);
        updateInfo = chalk.blue(` • ${parts.join(', ')}`);
      }
      
      console.log('');
      console.log(`${statusIcon} ${chalk.bold(id)} ${chalk.dim(`(priority: ${entry.priority})`)}${updateInfo}`);
      ...
      console.log(`  ${chalk.dim('Status:')}   ${entry.active ? chalk.green('active') : chalk.gray('inactive')}`);
    } catch (error) {
      console.log('');
      console.log(`${chalk.red('✖')} ${chalk.bold(id)} ${chalk.dim(`(priority: ${entry.priority})`)}`);
  ...
  if (installedEntries.length === 0) {
    ...
    console.log('');
    logger.info('Install skills: npx @bre/ai-setup skills');
  } else {
    // Group by catalog
    ...
    const removedSkills = new Set(diff.removed.map(s => `${s.catalog}-${s.skillName}`));

    // Display by catalog
    for (const [catalogId, skills] of byCatalog) {
      const catalogUpdateCount = skills.filter(([fullName]) => updatedSkills.has(fullName)).length;
      const catalogRemovedCount = skills.filter(([fullName]) => removedSkills.has(fullName)).length;
      
      let catalogStatus = '';
      if (catalogUpdateCount > 0 || catalogRemovedCount > 0) {
        const parts = [];
        if (catalogUpdateCount > 0) parts.push(chalk.blue(`${catalogUpdateCount} update${catalogUpdateCount === 1 ? '' : 's'}`));
        if (catalogRemovedCount > 0) parts.push(chalk.yellow(`${catalogRemovedCount} removed`));
        catalogStatus = ` • ${parts.join(', ')}`;
      }
      
      console.log('');
      console.log(chalk.bold(`  ${catalogId}`) + chalk.dim(` (${skills.length} skill${skills.length === 1 ? '' : 's'})`) + catalogStatus);
      
      for (const [fullName, skillData] of skills) {
        const installedDate = new Date(skillData.installedAt).toLocaleDateString();
        ...
        console.log(`      ${chalk.dim(`v${skillData.version} • ${installedDate}`)}`);
      }
    }

    console.log('');
    console.log(chalk.dim('─'.repeat(60)));
    console.log(`Total: ${installedEntries.length} skill${installedEntries.length === 1 ? '' : 's'} installed`);
    
    if (summary.hasChanges) {
      console.log('');
      const changeParts = [];
      if (summary.updatedCount > 0) changeParts.push(chalk.blue(`${summary.updatedCount} update${summary.updatedCount === 1 ? '' : 's'}`));
      if (summary.newCount > 0) changeParts.push(chalk.green(`${summary.newCount} new`));
      if (summary.removedCount > 0) changeParts.push(chalk.yellow(`${summary.removedCount} removed`));
  ...
  if (diff.new.length > 0) {
    ...
    for (const [catalogId, skills] of newByCatalog) {
      ...
      for (const skill of skills) {
        console.log(`    ${chalk.dim('○')} ${skill.skillName} ${chalk.dim('- ' + skill.skill.description)}`);
      }
      console.log('');
    }
  }
